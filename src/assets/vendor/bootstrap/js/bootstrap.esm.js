...
const getSelector = element => {
  ...
  if (!selector || selector === '#') {
    let hrefAttribute = element.getAttribute('href'); // The only valid content that could double as a selector are IDs or classNamees,
...
className Collapse extends BaseComponent {
  ...
  _getFirstLevelChildren(selector) {
    const children = SelectorEngine.find(className_NAME_DEEPER_CHILDREN, this._config.parent); // remove children if greater depth
...
className Tooltip extends BaseComponent {
  ...
  show() {
    ...
    tip.classNameList.add(className_NAME_SHOW$2); // If this is a touch-enabled device we add extra
    // empty mouseover listeners to the body's immediate children;
    // only needed because of broken event delegation on iOS
    // https://www.quirksmode.org/blog/archives/2014/02/mouse_event_bub.html
  ...
  hide() {
    ...
    tip.classNameList.remove(className_NAME_SHOW$2); // If this is a touch-enabled device we remove the extra
    // empty mouseover listeners we added for iOS support
  ...
  _createTipElement(content) {
    ...
    tip.classNameList.remove(className_NAME_FADE$2, className_NAME_SHOW$2); // todo: on v6 the following can be achieved with CSS only
